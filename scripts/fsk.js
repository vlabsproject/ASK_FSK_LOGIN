...
c.fillStyle = "white";
c.textAlign = "center";
c.fillText("Time", canvas.width/2, canvas.height-20);

// Draw y-axis label
c.save();
c.translate(20, canvas.height/2);
c.rotate(-Math.PI/2);
c.font = "20px Arial";
c.fillStyle = "white";
c.textAlign = "right";
c.fillText("Amplitude", 200, 0);
c.restore();
...
d.fillStyle = "#ffffff";
d.textAlign = "center";
d.fillText("Time", carrier.width/2, carrier.height - 10);
d.save();
d.rotate(-Math.PI/2);
d.textAlign = "right";
d.fillText("Amplitude", (-carrier.height/2)*0.1, 20);
d.restore();
  ...
  function ffskOutput(){
    ...
    if(bitStream[j]==0)
    {
        for (let i = j*fskSignal.width/8; i < ((j+1)*fskSignal.width)/8; i++)
            {
            f.lineTo(i, fskSignal.height/2 + Math.sin(i * ((freq_value*0.05) - (freq_value*0.03))) * amp_value * 10);  
            }
          // e.lineTo(output.width, output.height/2);
    }
    else
    {
        for (let i =j*fskSignal.width/8; i < ((j+1)*fskSignal.width)/8; i++) 
        {
        f.lineTo(i, fskSignal.height/2 + Math.sin(i * freq_value*0.05) * amp_value * 10);  
        }
      // e.lineTo(output.width, output.height/2);
    }
    }
    ...
    f.fillStyle = "#ffffff";
    f.textAlign = "center";
    f.fillText("Time", fskSignal.width/2, fskSignal.height - 10);
    f.save();
    f.rotate(-Math.PI/2);
    f.textAlign = "right";
    f.fillText("Amplitude", (-fskSignal.height/2)*0.1,20);
    f.restore();
}
// --------------------------------------------------DEMODULATED SIGNAL---------------------------------------------------------------
...
g.fillStyle = "white";
g.textAlign = "center";
g.fillText("Time", demodu.width/2, demodu.height-20);

// Draw y-axis label
g.save();
g.translate(20, demodu.height/2);
g.rotate(-Math.PI/2);
g.font = "20px Arial";
g.fillStyle = "white";
g.textAlign = "right";
g.fillText("Amplitude", 180, 0);
g.restore();
  for( let i=0; i<8; i++)
  {
      if(bitStream[i] == 0)
