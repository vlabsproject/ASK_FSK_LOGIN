...
for (let i = 0; i < carrier.width; i++) {
  d.lineTo(i, carrier.height/2 + Math.sin(i * freq_value*0.1) * amp_value * 10);  
}
...
function askOutput(){
    ...
    e.stroke();
     ...
     e.fillStyle = "#ffffff";
     e.textAlign = "center";
     e.fillText("Time", output.width/2, output.height - 10);
     e.save();
     e.rotate(-Math.PI/2);
     e.textAlign = "right";
     e.fillText("Amplitude", (-output.height/2)*0.1,20);
     e.restore();
  }
  // -------------------------------------------------FSK SIGNAL---------------------------------------------------------------------
function ffskOutput(){
